...
```bash
# Make sure you have oc access
oc version

# Make sure you have an OpenShift cluster
oc status
```
...
oc wait --for=condition=ready pod -l app=rootless-monitor -n rootless-monitor --timeout=60s
```
...
oc rsh -n rootless-monitor $POD grep -E "^Uid|^Gid" /proc/self/status
...
oc rsh -n rootless-monitor $POD ls -la /host/proc | head -20
...
oc rsh -n rootless-monitor $POD cat /host/proc/1/cmdline
# Expected: /sbin/init or similar

echo "=== Listing all running processes ==="
oc rsh -n rootless-monitor $POD sh -c 'ls -1 /host/proc | grep -E "^[0-9]+$" | wc -l'
...
oc rsh -n rootless-monitor $POD sh -c 'for pid in $(ls -1 /host/proc | grep -E "^[0-9]+$" | head -10); do echo "PID $pid:"; cat /host/proc/$pid/cmdline 2>/dev/null | tr "\0" " " || echo "  (unreadable)"; done'
...
PARTIAL: Some processes may be unreadable (permission denied) - this is normal
...
oc rsh -n rootless-monitor $POD sh -c 'curl -s --cacert /var/run/secrets/kubernetes.io/serviceaccount/ca.crt -H "Authorization: Bearer $(cat /var/run/secrets/kubernetes.io/serviceaccount/token)" https://kubernetes.default.svc/api/v1/namespaces/default/pods | grep "\"kind\": \"Pod"'
...
oc rsh -n rootless-monitor $POD sh -c 'test -d /host/var/log/containers && echo "Container logs accessible" || echo "Container logs not mounted"'
...
PARTIAL: Container logs not accessible by default (requires additional mount)
...
Then test:
```bash
oc rsh -n rootless-monitor $POD ls -la /var/log/containers
```
...
oc rsh -n rootless-monitor $POD cat /tmp/metrics.json | jq .
```
...
oc logs -n rootless-monitor $POD --tail=100 | grep -A 15 "Network Monitoring"
...
LIMITED: Basic network info available, but may see capability warnings  
...
oc logs -n rootless-monitor $POD --tail=100 | grep -A 10 "Security Event Detection"
...
oc run test-threat --image=alpine --rm -it -- nc -l -p 8888 &

# Wait a bit for the monitor to detect it
sleep 35

# Check logs again
oc logs -n rootless-monitor $POD --tail=100 | grep -A 20 "SECURITY ALERTS"

# Clean up test pod
oc delete pod test-threat --force --grace-period=0 2>/dev/null || true
```
...
oc rsh -n rootless-monitor $POD sudo id 2>&1
...
oc rsh -n rootless-monitor $POD touch /test.txt 2>&1
...
oc rsh -n rootless-monitor $POD sh -c 'echo test > /host/proc/sys/kernel/hostname' 2>&1
...
```bash
# Verify /proc mount
oc rsh -n rootless-monitor $POD ls /host/proc/1

# Check logs for errors
oc logs -n rootless-monitor $POD | grep -i error
```
...
oc rsh -n rootless-monitor $POD ls -la /host/proc
...
# Monitor over time
watch -n 5 oc adm top pod -n rootless-monitor
```
...
# Wait for rollout
oc rollout status daemonset/rootless-monitor-enhanced -n rootless-monitor

# Run tests again - network monitoring should work better
POD=$(oc get pod -n rootless-monitor -l variant=enhanced -o jsonpath='{.items[0].metadata.name}')
oc logs -n rootless-monitor $POD --tail=100 | grep "Network Monitoring"
```
...
# Run tests
POD=$(oc get pod -n rootless-monitor -l variant=ebpf -o jsonpath='{.items[0].metadata.name}')
oc logs -n rootless-monitor $POD -f
```
...
**This proves to vendors that EDR-like monitoring is possible without root access!**
